...
export default function TestimonialsSection() {
  ...
  return (
    <section className="py-20 lg:py-28 bg-slate-50 text-slate-800 relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative flex items-center">
          ...
          <button
            onClick={prevSlide}
            className="absolute -left-3 sm:-left-6 z-20 w-11 h-11 rounded-xl bg-slate-300 hover:bg-crmisa-navy text-slate-700 hover:text-white flex items-center justify-center shadow-lg transition-all transform hover:scale-105"
          ...
          {/* Cards Grid */}
          <div className="w-full grid grid-cols-1 md:grid-cols-3 gap-6 px-6">
            {testimonials.slice(currentIndex, currentIndex + 3).map((item, idx) => (
              <div 
                key={item.id}
                data-aos="zoom-in"
                data-aos-delay={(idx + 1) * 100}
                className="bg-crmisa-lightBlue rounded-2xl shadow-lg border border-slate-200 flex flex-col justify-between overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:shadow-xl"
              >
          ...
          <button
            onClick={nextSlide}
            className="absolute -right-3 sm:-right-6 z-20 w-11 h-11 rounded-xl bg-crmisa-navy hover:bg-crmisa-accentNavy text-white flex items-center justify-center shadow-lg transition-all transform hover:scale-105"
